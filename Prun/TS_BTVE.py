...
class TS_BTVE(object):
    def __init__(self, bounds):
        ...
        log_file = Path('./logs/%s.log' %('bese'))  #logging as SuccessiveBlockElimination (sbe)
        logging.basicConfig(filename = log_file, format='%(asctime)s : %(message)s', level=logging.INFO)
        self.log = logging.getLogger('bese')
    ...
    def update(self, state, revenue, ind):
        ...
        arm_sem = 1 #we keep this high to 
        arm_var = 1
        arm_pvar = self.p_var
        #arm_pvar = self.p_var
